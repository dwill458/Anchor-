...
export const ThreadRing: React.FC<ThreadRingProps> = ({
  ...
  const progress = useSharedValue(reduceMotionEnabled ? value / 100 : 0);

  useEffect(() => {
    if (reduceMotionEnabled) {
      progress.value = Math.min(Math.max(value / 100, 0), 1);
      return;
    }

    progress.value = 0;
    progress.value = withTiming(Math.min(Math.max(value / 100, 0), 1), {
      duration: 900,
      easing: Easing.bezier(0.22, 1, 0.36, 1),
    });
  }, [value, reduceMotionEnabled, progress]);
  ...
  const activeLength = (Math.min(Math.max(value, 0), 100) / 100) * MAX;
