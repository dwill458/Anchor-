...
describe('ChargeSetupScreen', () => {
  ...
  it('preselects and starts a matching location preset without changing saved defaults', async () => {
    ...
    expect(screen.getByText('Gym · 5 min · No Voice • Silence')).toBeTruthy();
  ...
  it('preselects the initial deep duration without auto-starting when auto-start is off', () => {
    ...
    expect(
      screen.getByLabelText('Deep Prime duration').props.accessibilityState,
    ).toEqual(
  ...
  it('renders the enhanced anchor artwork when available', () => {
    ...
    expect(
      screen.getByText('https://example.com/enhanced-anchor.png'),
    ).toBeTruthy();
  });
