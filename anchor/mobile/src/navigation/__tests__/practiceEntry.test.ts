...
function buildDependencies(overrides: Record<string, unknown> = {}) {
  return {
    ...
    defaultDeepChargeDurationSeconds: 120,
...
describe('startPractice', () => {
  it('sends an already-primed anchor straight into the ritual, skipping the setup screen', () => {
  ...
  it('shows the Choose Your Prime setup screen only for an anchor that has never been primed', () => {
    const unprimedAnchor = { ...anchor, isCharged: false, activationCount: 0 };
    const dependencies = buildDependencies({
      getAnchorById: (id: string) => (id === unprimedAnchor.id ? unprimedAnchor : undefined),
    });

    expect(startPractice({
      mode: 'deepPrime',
      anchorId: unprimedAnchor.id,
      source: 'practice_hero',
    }, dependencies)).toBe(true);
