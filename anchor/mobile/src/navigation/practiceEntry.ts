...
import { isFirstPrimeForAnchor } from '@/utils/anchorPriming';
...
export interface StartPracticeDependencies {
  ...
  defaultDeepChargeDurationSeconds: number;
...
export function startPractice(
  ...
  switch (request.mode) {
    case 'deepPrime': {
      // The "Choose Your Prime" duration picker is an onboarding moment for
      // the anchor's first ever charge. Anchors that have already been
      // primed skip straight into the ritual with the user's saved duration.
      const needsPrimeSetup = request.durationSeconds == null && isFirstPrimeForAnchor(anchor);
      target = needsPrimeSetup
        ? {
          route: 'ChargeSetup',
          params: {
            anchorId,
        ...
        : {
          ...
          params: {
            anchorId,
            ritualType: 'ritual',
            durationSeconds: request.durationSeconds ?? dependencies.defaultDeepChargeDurationSeconds,
            audioConfiguration,
            returnTo: 'practice',
            source: request.source,
          },
