...
export function useV2PracticeModel(anchor: Anchor | null, suppliedRecommendation?: V2RecommendationContext | null) {
  ...
  useEffect(() => {
    if (suppliedRecommendation && refreshIndex === 0) {
      setRecommendation(suppliedRecommendation);
  ...
  }, [anchor?.id, suppliedRecommendation, refreshIndex]);
  ...
  return useMemo(() => {
    ...
    return {
      anchor,
      thread: anchor ? toThreadPresentation(anchor) : null,
      vision: anchor ? toV2HomeVisionState(visionModel.state) : { state: 'none' as const },
      recommendation,
      recommendationError,
