...
function headline(bundle: V2ThreadEventBundle) {
  ...
  return threadEventLabel(eventType);
}
...
export function V2ThreadEventCelebration({ bundle, visible, anchorSvg, category, reducedMotion = false, onPresented, onAcknowledge, onDismiss, testID = 'v2-thread-event-celebration' }: Props) {
  ...
  return <Modal visible={visible} transparent animationType={reducedMotion ? 'fade' : 'slide'} onRequestClose={onDismiss} testID={testID}>
    <View style={styles.overlay}><View style={[styles.chamber, dark ? styles.darkChamber : styles.lightChamber]}>
      ...
      <Text style={[styles.eyebrow, { color: categoryColor }]}>{event.metadata.stageName ?? threadEventLabel(event.eventType).toUpperCase()}</Text>
      ...
      {persistedThreadValue !== null ? <Text style={[styles.threadValue, { color: categoryColor }]}>Consistency {persistedThreadValue}%</Text> : null}
...
const styles = StyleSheet.create({
  ...
  chamber: { minHeight: 520, overflow: 'hidden', paddingHorizontal: spacing[6], paddingTop: spacing[8], paddingBottom: spacing[7] }, lightChamber: { backgroundColor: colors.canvas }, darkChamber: { backgroundColor: colors.ink.base },
