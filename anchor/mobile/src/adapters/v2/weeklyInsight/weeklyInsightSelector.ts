...
export function selectWeeklyInsight(facts: WeeklyInsightFacts): WeeklyInsightSnapshot {
  ...
  if (releasedAnchor && (releasedAnchor.releasedThisWeek || facts.modeCounts.release > 0)) {
    ...
    return {
      ...
      evidence: [
        ...
        [`${finalScore}`, 'Final Consistency', 'Snapshot'],
  ...
  if (evolvedAnchor || evolutionEvent) {
    ...
    return {
      ...
      evidence: [
        ...
        [`${facts.endThread}`, 'Consistency', 'At transition'],
        [`${totalPractices}`, 'Practices', 'This week'],
      ],
      comparison: 'This stage change is preserved in your Anchor history, not inferred from the recap itself.',
      comparisonTone: 'neutral',
      interpretation:
  ...
  if (milestoneEvent) {
    ...
    return {
      ...
      evidence: [
        ...
        [`${totalPractices}`, 'Practices', 'This week'],
        [`+${Math.max(0, facts.authoritativeThreadDelta)}`, 'Consistency', `${facts.startThread} → ${facts.endThread}`],
      ],
  ...
  if (hasRecoveryEvent || isStrongRecovery) {
    ...
    return {
      ...
      support: `Your Consistency dipped early in the week, then recovered after ${practicesAfter} completed Practices.`,
      ...
      comparison: 'Your strongest Consistency recovery in the last four completed weeks.',
      comparisonTone: 'positive',
      interpretation:
  ...
  if (totalPractices === 0) {
    return {
      ...
      evidence: [
        ...
        [`${facts.authoritativeThreadDelta >= 0 ? '+' : ''}${facts.authoritativeThreadDelta}`, 'Consistency', `${facts.startThread} → ${facts.endThread}`],
  ...
  if (daysInactive >= V1_INSIGHT_THRESHOLDS.returnDaysInactive) {
    ...
    return {
      ...
      support: `After ${daysInactive} days away, you completed ${totalPractices} Practices and began rebuilding Consistency with your ${anchorName} Anchor.`,
      accentColor: WEEKLY_INSIGHT_COLORS[primaryAccentKey],
      accentKey: primaryAccentKey,
      visualType: 'activity',
      visualData: {
      ...
      evidence: [
        ...
        [`+${delta}`, 'Consistency', 'Since return'],
      ],
  ...
  if (
    ...
    return {
      ...
      evidence: [
        ...
        [`+${delta}`, 'Consistency', 'Since creation'],
      ],
  ...
  if (totalPractices === 1) {
    ...
    return {
      ...
      headline: 'This was a quieter week.',
      support: 'You practiced once and your Consistency dipped slightly. There is not enough activity here to claim a broader pattern.',
      accentColor: WEEKLY_INSIGHT_COLORS.neutral,
      accentKey: 'neutral',
      visualType: 'activity',
      visualData: {
      ...
      evidence: [
        ...
        [`${delta >= 0 ? '+' : ''}${delta}`, 'Consistency', `${facts.startThread} → ${facts.endThread}`],
  ...
  if (isChartAlignment && facts.chartContext) {
    ...
    return {
      ...
      evidence: [
        ...
        [`+${Math.max(0, facts.authoritativeThreadDelta)}`, 'Consistency', `${anchorName} Anchor`],
      ],
  ...
  if (isDepth) {
    ...
    return {
      ...
      support: 'You practiced fewer times than last week, but Deep Focus made up most of your reinforcement.',
      ...
      evidence: [
        [`${deepCount}`, 'Deep Focus', `Of ${totalPractices} Practices`],
        [`+${Math.max(0, facts.authoritativeThreadDelta)}`, 'Consistency', `${facts.startThread} → ${facts.endThread}`],
        [diffLabel, 'Practices', 'Vs last week'],
      ],
      comparison: 'Two fewer Practices than last week, with a larger Consistency gain.',
      comparisonTone: 'positive',
  ...
  if (hasDominantLead && topAnchor) {
    ...
    return {
      ...
      support: `${topAnchor.practiceCountInWeek} of your ${totalPractices} Practices reinforced the same intention, moving its Consistency from ${facts.startThread} to ${facts.endThread}.`,
  ...
  if (
    ...
    return {
      ...
      headline: 'You kept your Consistency steady.',
      support: `You practiced across ${facts.activeDaysCount} different days and avoided a meaningful dip, even without adding more sessions.`,
      accentColor: WEEKLY_INSIGHT_COLORS[primaryAccentKey],
      accentKey: primaryAccentKey,
      visualType: 'activity',
      visualData: {
      ...
      evidence: [
        ...
        [`${facts.startThread} → ${facts.endThread}`, 'Consistency', 'Stable movement'],
      ],
      ...
      nextDirection: 'Keep the same rhythm. Short Focus sessions are enough to stay consistent.',
      detailedActivity,
      createdAt: snapshotCreatedAt,
    };
  }
  ...
  if (isSplit) {
    return {
      ...
      comparisonTone: 'neutral',
      interpretation: 'Broad practice kept several intentions moving, even without deep concentration on a single Anchor.',
  ...
  return {
    ...
    evidence: [
      [`${totalPractices}`, 'Practices', 'Completed'],
      [`${facts.activeDaysCount}`, 'Active days', 'Across week'],
      [`${facts.startThread} → ${facts.endThread}`, 'Consistency', 'Movement'],
    ],
