...
const CONTROLNET_METHOD_BY_STYLE: Record<AIStyle, StyleConfig['method']> = {
  ...
  screenprint: 'canny',
  monoprint: 'lineart',
  charcoal_field: 'lineart',
  graphite_study: 'canny',
  stone_relief: 'canny',
  porcelain: 'canny',
  copper_patina: 'canny',
  topographic: 'lineart',
  botanical_etching: 'lineart',
  mineral_bloom: 'lineart',
  dreamscape: 'lineart',
  soft_monument: 'canny',
  street_mark: 'lineart',
  collage_archive: 'canny',
  lightfield: 'lineart',
};
...
const CONTROLNET_CATEGORY_BY_STYLE: Record<AIStyle, StyleConfig['category']> = {
  ...
  cut_paper: 'geometric',
  risograph: 'geometric',
  screenprint: 'geometric',
  monoprint: 'organic',
  charcoal_field: 'organic',
  graphite_study: 'geometric',
  stone_relief: 'geometric',
  porcelain: 'geometric',
  copper_patina: 'hybrid',
  topographic: 'geometric',
  botanical_etching: 'organic',
  mineral_bloom: 'hybrid',
  dreamscape: 'hybrid',
  soft_monument: 'geometric',
  street_mark: 'hybrid',
  collage_archive: 'hybrid',
  lightfield: 'hybrid',
};
...
const STRUCTURE_THRESHOLDS = {
  ...
  artistic: 0.7, // 70-85% = "More Artistic"
...
export async function enhanceSigilWithAI(
  ...
  if (geminiService.isAvailable()) {
    try {
      ...
      logger.error(
        `[AIEnhancer] Gemini 3 failed, will NOT fall back to Replicate to enforce Google API. Error: ${errMsg}`,
        error instanceof Error ? error : new Error(errMsg)
      );
      throw new Error(`Google Generation API Failed: ${errMsg}`);
    }
  } else {
...
export async function enhanceSigilWithControlNet(
  ...
  try {
    ...
    const conditioningScale =
      styleConfig.conditioning_scale || CONTROLNET_CONFIG.conditioning_scale;
    ...
    const bestVariationIndex = variations.reduce(
      (best, v, i) =>
        v.structureMatch.combinedScore > variations[best].structureMatch.combinedScore ? i : best,
      0
    );
