...
export class WeeklyInsightService {
  async generate(
    userId: string,
    input: { anchorId?: string | null; timeZone?: string; now?: Date }
  ) {
    ...
    if (anchorId) {
      const owned = await prisma.anchor.findFirst({
    ...
    const weeklySessions = sessions.filter(session => {
      ...
      return (
        key >= computeLocalDateKey(weekStart, timeZone) &&
        key <= computeLocalDateKey(weekEnd, timeZone)
      );
    });
    ...
    const [events, views, courseEvents] = await Promise.all([
      prisma.threadEventLedger.findMany({
        where: {
          userId,
      ...
      prisma.visionView.findMany({
        where: { userId, viewedAt: { gte: rangeStart, lte: rangeEnd } },
        select: { viewedAt: true, vision: { select: { anchorId: true, title: true } } },
      }),
      prisma.courseEvent.findMany({
        where: { userId, occurredAt: { gte: rangeStart, lte: rangeEnd } },
        orderBy: { occurredAt: 'asc' },
        select: { eventType: true, occurredAt: true, waypoint: { select: { title: true } } },
      }),
    ]);

    const modeCounts: Record<InsightMode, number> = {
      focus: 0,
      deep_prime: 0,
      visualize: 0,
      release: 0,
    ...
    weeklyMovements.forEach(movement => {
      movementByDay[dayIndex(movement.completedAt, timeZone)] = movement.afterStrength;
    });
    const firstMovement = weeklyMovements[0];
    const lastMovement = weeklyMovements[weeklyMovements.length - 1];
    const threadPoints = movementByDay.map(
      (value, index) =>
        value ?? (index ? movementByDay[index - 1] : (firstMovement?.beforeStrength ?? 0))
    );
    ...
    const facts: Fact = {
      ...
      completedDateLabel: 'Current week',
      sessions: weeklySessions.map(session => ({
        ...
        dayOfWeekIndex: dayIndex(session.completedAt, timeZone),
        isLinkedToWaypoint: Boolean(session.waypointId),
      })),
      activeDaysCount: activeDays.filter(Boolean).length,
      activeDays,
      totalDurationSeconds: weeklySessions.reduce(
        (total, session) => total + session.completedDurationSeconds,
        0
      ),
      modeCounts: {
        focus: modeCounts.focus,
        deepPrime: modeCounts.deep_prime,
        visualize: modeCounts.visualize,
        release: modeCounts.release,
      },
      threadPoints,
      startThread: firstMovement?.beforeStrength ?? null,
      endThread: lastMovement?.afterStrength ?? null,
      authoritativeThreadDelta: weeklyMovements.length
        ? weeklyMovements.reduce((total, movement) => total + movement.delta, 0)
        : null,
      canonicalEvents: [
        ...events.map(event => ({
          type: event.eventType,
          dayLabel: dayLabel(dayIndex(event.occurredAt, timeZone)),
        ...
        ...courseEvents.map(event => ({
          ...
          dayLabel: dayLabel(dayIndex(event.occurredAt, timeZone)),
          occurredAt: event.occurredAt.toISOString(),
          metadata: event.waypoint?.title ? { waypointTitle: event.waypoint.title } : undefined,
        })),
      ],
      anchors: anchors.map(anchor => ({
        ...
        practiceCountInWeek: weeklySessions.filter(session => session.anchorId === anchor.id)
          .length,
        releasedThisWeek: Boolean(
          anchor.releasedAt && anchor.releasedAt >= weekStart && anchor.releasedAt <= weekEnd
        ),
      })),
      visionContext: {
        revisitsCount: views.filter(view => !anchorId || view.vision.anchorId === anchorId).length,
      },
      evidenceAvailability: {
        thread: weeklyMovements.length > 0,
        course: courseEvents.length > 0,
        vision: views.length > 0,
      },
      timeZone,
    };
    const total = weeklySessions.length;
    const headline = total
      ? `You completed ${total} practice${total === 1 ? '' : 's'} this week.`
      : 'No Practice was recorded this week.';
    const snapshot = {
      weekLabel: weekLabelText,
      completedDateLabel: 'Current week',
      ruleType: total ? 'ACTIVITY' : 'QUIET',
      headline,
      support: 'Based on your recorded Anchor activity.',
      accentColor: '#7A6252',
      accentKey: 'neutral',
      visualType: 'activity',
      visualData: {
        ...
        activeDays,
        activeDaysCount: activeDays.filter(Boolean).length,
        completedPracticesCount: total,
      },
      evidence: [
        [String(total), 'PRACTICES', 'Recorded'],
        [String(activeDays.filter(Boolean).length), 'ACTIVE DAYS', 'Recorded'],
        [
          lastMovement ? `${lastMovement.afterStrength}` : 'Not recorded',
          'THREAD',
          lastMovement ? 'Authoritative' : 'Not recorded',
        ],
      ],
      comparison: 'Not recorded',
      comparisonTone: 'neutral',
      interpretation: headline,
      nextDirection: 'Return when you are ready.',
      detailedActivity: {
        totalPractices: total,
        activeDays: activeDays.filter(Boolean).length,
        totalMinutes: Math.floor(
          weeklySessions.reduce((sum, session) => sum + session.completedDurationSeconds, 0) / 60
        ),
        threadStart: firstMovement?.beforeStrength ?? null,
        threadEnd: lastMovement?.afterStrength ?? null,
        threadPoints,
        modeDistribution: [],
        connectedActivity: [],
        canonicalEvents: events.map(event => ({
          label: event.eventType,
          day: dayLabel(dayIndex(event.occurredAt, timeZone)),
        })),
      },
      facts,
