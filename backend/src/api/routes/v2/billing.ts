...
const trialActivationLimiter = rateLimit({
  ...
  message: {
    ...
    error: {
      ...
      message: 'Too many trial activation attempts. Please try again later.',
    },
  },
});
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (initialState === 'EXPIRED')
        throw new AppError('This trial has already been used.', 409, 'TRIAL_ALREADY_USED');
      if (initialState === 'NOT_STARTED') {
        ...
        await prisma.user.updateMany({
          where: { id: existing.id, trialStartedAt: null },
      ...
      res.json({
        success: true,
        data: {
          trialState: getTrialState(user.trialStartedAt),
